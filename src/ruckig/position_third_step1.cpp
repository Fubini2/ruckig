...
void PositionThirdOrderStep1::time_all_vel(ProfileIter& profile, double vMax, double vMin, double aMax, double aMin, double jMax, bool) const {
    ...
    // ACC1_VEL
    double temp0 = a0_a0 / (2 * jMax_jMax) + (vMax - v0) / jMax;
    if (temp0 < 0.0 && temp0 > -DBL_EPSILON) {
        temp0 = 0.0;
    }
    const double t_acc0 = std::sqrt(temp0);
    ...
    // ACC0_VEL
    double temp1 = af_af / (2 * jMax_jMax) + (vMax - vf) / jMax;
    if (temp1 < 0.0 && temp1 > -DBL_EPSILON) {
        temp1 = 0.0;
    }
    const double t_acc1 = std::sqrt(temp1);
...
void PositionThirdOrderStep1::time_all_none_acc0_acc1(ProfileIter& profile, double vMax, double vMin, double aMax, double aMin, double jMax, bool return_after_found) const {
    ...
    //for some input data solver produces roots slightly outside time range
    //if jMax is much larger than aMax (~ 1e5)
    const double SolverEps = 1e-9;

    const double h2_none = (a0_a0 - af_af) / (2 * jMax) + (vf - v0);
    const double h2_h2 = h2_none * h2_none;
    const double t_min_none = (a0 - af) / jMax * (1 - SolverEps);
    const double t_max_none = (aMax - aMin) / jMax * (1 + SolverEps);
    ...
    const double t_min_acc0 = (aMax - af) / jMax * (1 - SolverEps);
    const double t_max_acc0 = (aMax - aMin) / jMax * (1 + SolverEps);
    ...
    const double t_min_acc1 = (aMin - a0) / jMax * (1 - SolverEps);
    const double t_max_acc1 = (aMax - a0) / jMax * (1 + SolverEps);
