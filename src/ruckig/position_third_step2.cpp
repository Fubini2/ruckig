...
bool PositionThirdOrderStep2::time_acc0_acc1_vel(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    ...
    if ((2 * (aMax - aMin) + ad) / jMax < tf) {
        ...
        if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC0_ACC1_VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
            return true;
        }
    }
    ...
    if ((-a0 + 4 * aMax - af) / jMax < tf) {
        ...
        if (profile.check_with_timing<ControlSigns::UDUD, ReachedLimits::ACC0_ACC1_VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
            return true;
        }
    }
...
bool PositionThirdOrderStep2::time_acc1_vel(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    // Profile UDDU
    {
        ...
        for (double t : roots) {
            ...
            if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC1_VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
                return true;
            }
        }
    }
    ...
    // Profile UDUD
    {
        ...
        for (double t : roots) {
            ...
            if (profile.check_with_timing<ControlSigns::UDUD, ReachedLimits::ACC1_VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
                return true;
            }
        }
    }
...
bool PositionThirdOrderStep2::time_acc0_vel(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    ...
    // Profile UDDU
    {
        ...
        for (double t : roots) {
            ...
            if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC0_VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
                return true;
            }
        }
    }
    ...
    // Profile UDUD
    {
        ...
        for (double t : roots) {
            ...
            if (profile.check_with_timing<ControlSigns::UDUD, ReachedLimits::ACC0_VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
                return true;
            }
        }
    }
...
bool PositionThirdOrderStep2::time_vel(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    ...
    if (std::abs(v0) < DBL_EPSILON && std::abs(a0) < DBL_EPSILON && std::abs(vf) < DBL_EPSILON && std::abs(af) < DBL_EPSILON) {
        ...
        for (double t : roots) {
            ...
            if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::VEL>(tf, jMax, vMax, vMin, aMax, aMin)) {
                return true;
            }
        }
    } else {
        ...
        const auto check_root = [&](double t) {
            ...
            {
                double h1 = std::sqrt((a0_a0 + af_af) / (2 * jMax_jMax) + (2 * a0 * t + jMax * t * t - vd) / jMax);
                double orig = -pd - (2 * a0_p3 + 4 * af_p3 + 24 * a0 * jMax * t * (af + jMax * (h1 + t - tf)) + 6 * a0_a0 * (af + jMax * (2 * t - tf)) + 6 * (a0_a0 + af_af) * jMax * h1 + 12 * af * jMax * (jMax * t * t - vd) + 12 * jMax_jMax * (jMax * t * t * (h1 + t - tf) - tf * v0 - h1 * vd)) / (12 * jMax_jMax);
                double deriv_newton = -(a0 + jMax * t) * (3 * (h1 + t) - 2 * tf + (a0 + 2 * af) / jMax);
                if (!std::isnan(orig) && !std::isnan(deriv_newton) && std::abs(deriv_newton) > DBL_EPSILON) {
                    t -= orig / deriv_newton;
                }

                h1 = std::sqrt((a0_a0 + af_af) / (2 * jMax_jMax) + (2 * a0 * t + jMax * t * t - vd) / jMax);
            ...
            return profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::VEL>(tf, jMax, vMax, vMin, aMax, aMin);
        };
    ...
    // Profile UDUD
    {
        ...
        const auto check_root = [&](double t) {
            ...
            return profile.check_with_timing<ControlSigns::UDUD, ReachedLimits::VEL>(tf, jMax, vMax, vMin, aMax, aMin);
        };
...
bool PositionThirdOrderStep2::time_acc0_acc1(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    if (std::abs(a0) < DBL_EPSILON && std::abs(af) < DBL_EPSILON) {
        ...
        return profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC0_ACC1>(tf, jf, vMax, vMin, aMax, aMin, jMax);
    }
    ...
    //     if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC0_ACC1>(tf, jMax, vMax, vMin, aMax, aMin)) {
    ...
    // UDDU
    {
        ...
        if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC0_ACC1>(tf, jf, vMax, vMin, aMax, aMin, jMax)) {
            return true;
        }
    }
...
bool PositionThirdOrderStep2::time_acc1(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    ...
    // Case UDDU
    {
        ...
        if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::ACC1>(tf, jMax, vMax, vMin, aMax, aMin)) {
            return true;
        }
    }
    ...
    // Case UDUD
    {
        ...
        if (profile.check_with_timing<ControlSigns::UDUD, ReachedLimits::ACC1>(tf, jMax, vMax, vMin, aMax, aMin)) {
            return true;
        }
    }
...
bool PositionThirdOrderStep2::time_none(Profile& profile, double vMax, double vMin, double aMax, double aMin, double jMax) {
    if (std::abs(v0) < DBL_EPSILON && std::abs(a0) < DBL_EPSILON && std::abs(af) < DBL_EPSILON) {
        ...
        if (profile.check_with_timing<ControlSigns::UDDU, ReachedLimits::NONE>(tf, jf, vMax, vMin, aMax, aMin, jMax)) {
            return true;
        }
    }
...
bool PositionThirdOrderStep2::get_profile(Profile& profile) {
    ...
    if (minimize_jerk && (time_none_smooth(profile, vMax, vMin, aMax, aMin, jMax) || time_none_smooth(profile, vMin, vMax, aMin, aMax, -jMax))) {
        return true;
    }
